...
public class OperatorFactoryImpl implements OperatorFactory {
	...
	protected Gettable<?> getGettable(Gettable<?> ref, Class<?> type){
		if(type.isAssignableFrom( ref.type() )) {
			...
			return new GettableWrapper(ref,  type);
		}
...
//		throw new DslBuildException(ref + " is not Ref or not supported unchecked mode.");
	...
	public class GettableWrapper implements Gettable {
		...
		public GettableWrapper(Gettable<?> original, Class<?> type) {
		...
		public Object get(Context context) {
			...
			throw new IllegalArgumentException( "Type mismatch :"
					+ type.getName() + " is expected, but  " 
					 + original + " type is " + val.getClass().getName() );
		}
